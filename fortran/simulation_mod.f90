module simulation_mod
  ...
  subroutine compute_tentative_velocity(u, v, f, g, flag, del_t)
    real u(0:imax+1, 0:jmax+1), v(0:imax+1, 0:jmax+1), f(0:imax+1, 0:jmax+1),  &
         g(0:imax+1, 0:jmax+1)
    ...
    do i = 1, (imax-1)
      do j = 1, jmax
        ...
        if (toLogical(iand(flag(i,j), C_F)) .and.                              &
            toLogical(iand(flag(i+1,j), C_F))) then
    ...
    do i = 1, imax
       do j = 1, (jmax-1)
          ! only if both adjacent cells are fluid cells
          if (toLogical(iand(flag(i,j), C_F)) .and.                            &
              toLogical(iand(flag(i,j+1), C_F))) then
  ...
  end subroutine compute_tentative_velocity
  ...
  subroutine update_velocity(u, v, f, g, p, flag, del_t)
    real u(0:imax+1, 0:jmax+1), v(0:imax+1, 0:jmax+1), f(0:imax+1, 0:jmax+1),  &
         g(0:imax+1, 0:jmax+1), p(0:imax+1, 0:jmax+1)
    integer flag(0:imax+1, 0:jmax+1)
    real, intent(in) :: del_t
  ...
  end function set_timestep_interval
